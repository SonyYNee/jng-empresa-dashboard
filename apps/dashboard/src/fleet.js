...
export function parseOdometer(value) {
  if (typeof value === 'number')
    return Number.isFinite(value) && value >= 0 && value <= 10000000 ? value : NaN;
  if (typeof value !== 'string') return NaN;
  const text = value.trim();
  if (/^\d{1,3}(?:\.\d{3})+(?:,\d{1,2})?$/.test(text))
    return parseOdometer(Number(text.replaceAll('.', '').replace(',', '.')));
...
export function fuelSummary(records, capacity, odometer) {
  ...
  if (anchor && capacity && (average || odometer === anchor.odometer)) {
    ...
    if (raw < 0 || raw > capacity + 0.1)
      warning = 'Saldo incompatível com a capacidade. Confira quilometragem e abastecimentos.';
...
export function initFleet(db, { session, body, json, validPhoto }) {
  ...
  const num = (value, min, max) => {
    if (typeof value !== 'number' && typeof value !== 'string')
      fail('Preencha os campos numéricos.');
    if (String(value).trim() === '') fail('Preencha os campos numéricos.');
    const n = Number(String(value).replace(',', '.'));
    if (!Number.isFinite(n) || n < min || n > max)
      fail('Valor numérico fora do intervalo permitido.');
    return n;
  };
  const integer = (value, min, max) => {
    const n = num(value, min, max);
    if (!Number.isInteger(n)) fail('Informe um número inteiro.');
    return n;
  };
  const kilometres = (value) => {
    const result = parseOdometer(value);
    if (!Number.isFinite(result))
      fail('Informe a quilometragem entre 0 e 10.000.000 km. Exemplo: 150000 ou 150.000.');
    return result;
  };
  const cents = (value) => Math.round(num(value, 0.01, 100000000) * 100);
  const text = (value, max = 120) => {
    if (typeof value !== 'string' || !value.trim() || value.trim().length > max)
      fail('Preencha a descrição corretamente.');
    return value.trim();
  };
  ...
  async function summary(vehicle) {
    const rows = (
      await db.all('SELECT * FROM refills WHERE vehicle_id=$1 ORDER BY date,odometer,id', [
        vehicle.id,
      ])
    ).map((row) => ({ ...row, full: row.tank_full ?? row.full }));
    const entries = await db.all(
      'SELECT * FROM vehicle_entries WHERE vehicle_id=$1 ORDER BY date DESC,id DESC',
      [vehicle.id],
    );
    const maintenance = await db.all(
      'SELECT * FROM maintenance WHERE vehicle_id=$1 ORDER BY date,id',
      [vehicle.id],
    );
    ...
    const variable =
      entries.filter((r) => r.kind === 'cost').reduce((sum, r) => sum + r.amount, 0) +
      rows.reduce((sum, r) => sum + r.total, 0) +
      maintenance.filter((r) => r.status === 'done').reduce((sum, r) => sum + r.amount, 0);
    return {
      ...vehicle,
      ...
      fuel: fuelSummary(
        rows.filter((r) => r.kind === 'fuel'),
        vehicle.fuel_capacity,
        vehicle.odometer,
      ),
      arla:
        vehicle.arla_capacity > 0
          ? fuelSummary(
              rows.filter((r) => r.kind === 'arla'),
              vehicle.arla_capacity,
              vehicle.odometer,
            )
          : null,
      refills: rows.reverse(),
      entries,
      maintenance,
      maintenance_months: maintenanceForecast(maintenance),
    };
  }
  return async (req, res, pathname) => {
    ...
    try {
      ...
      if (req.method === 'GET' && pathname === '/api/fleet') {
        ...
        json(res, 200, { canManage, vehicles });
        return true;
      }
      if (req.method !== 'POST') fail('Rota não encontrada.', 404);
      if (!canManage)
        fail(
          'Somente proprietários e responsáveis pela operação ou frota podem fazer alterações.',
          403,
        );
      ...
      if (pathname === '/api/fleet/vehicle') {
        ...
        if (!/^[A-Z]{3}[0-9][A-Z0-9][0-9]{2}$/.test(plate))
          fail('Informe uma placa válida, como ABC1D23 ou ABC1234.');
        ...
        if (existing && odometer < existing.odometer)
          fail('A quilometragem não pode ser menor que a atual.');
        const fuel = num(data.fuel_capacity, 1, 5000),
          arla = num(data.arla_capacity, 0, 1000);
        if (
          existing?.arla_capacity &&
          !arla &&
          (await db.get("SELECT id FROM refills WHERE vehicle_id=$1 AND kind='arla' LIMIT 1", [id]))
        )
          fail('Mantenha a capacidade do ARLA para preservar os cálculos do histórico.');
        if (
          !Array.isArray(data.photos) ||
          data.photos.length > 5 ||
          data.photos.some((photo) => photo === null || !validPhoto(photo))
        )
          fail('Envie até cinco fotos JPG ou PNG válidas.');
        if (await db.get('SELECT id FROM vehicles WHERE plate=$1 AND id<>$2', [plate, id || 0]))
          fail('Esta placa já está cadastrada.', 409);
        const values = [
          plate,
          model,
          year,
        ...
        if (id)
          await db.run(
            'UPDATE vehicles SET plate=$1,model=$2,year=$3,seats=$4,odometer=$5,fuel_capacity=$6,arla_capacity=$7,photos=$8 WHERE id=$9',
            [...values, id],
          );
        else
          await db.run(
            'INSERT INTO vehicles(plate,model,year,seats,odometer,fuel_capacity,arla_capacity,photos) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
            values,
          );
        json(res, id ? 200 : 201, { ok: true });
        return true;
      }
      ...
      if (pathname === '/api/fleet/delete') {
        if (data.confirm_plate !== vehicle.plate)
          fail('Digite a placa do veículo para confirmar a exclusão.');
        ...
        const odometer = kilometres(data.odometer);
        const litres = num(data.litres, 0.001, capacity);
        const price = Math.round(num(data.price, 0.001, 1000) * 1000);
        if (typeof data.full !== 'boolean') fail('Informe se completou o tanque.');
        const previous = await db.get(
          'SELECT * FROM refills WHERE vehicle_id=$1 ORDER BY date DESC,odometer DESC,id DESC LIMIT 1',
          [vehicleId],
        );
        if (previous && (refillDate < previous.date || odometer < previous.odometer))
          fail('Registre os abastecimentos em ordem de data e quilometragem.');
        const same = await db.get(
          'SELECT id FROM refills WHERE vehicle_id=$1 AND kind=$2 AND odometer=$3',
          [vehicleId, data.kind, odometer],
        );
        if (same) fail('Já existe abastecimento desse produto nesta quilometragem.', 409);
        await db.transaction(async (client) => {
          await client.query(
            'INSERT INTO refills(vehicle_id,kind,date,odometer,litres,price,total,tank_full) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
            [
              vehicleId,
              data.kind,
              refillDate,
              ...
              litres,
              price,
              Math.round((litres * price) / 10),
              Number(data.full),
            ],
          );
          await client.query(
            'UPDATE vehicles SET odometer = CASE WHEN odometer > $1 THEN odometer ELSE $1 END WHERE id=$2',
            [odometer, vehicleId],
          );
        });
        ...
        await db.run(
          'INSERT INTO vehicle_entries(vehicle_id,kind,description,date,amount) VALUES ($1,$2,$3,$4,$5)',
          [vehicleId, data.kind, text(data.description), date(data.date), cents(data.amount)],
        );
        ...
        if (data.status === 'done' && recurrence)
          fail('Para recorrência, cadastre uma manutenção agendada e marque-a como realizada.');
        await db.run(
          'INSERT INTO maintenance(vehicle_id,description,date,amount,status,recurrence) VALUES ($1,$2,$3,$4,$5,$6)',
          [
            vehicleId,
            text(data.description),
            date(data.date, data.status === 'planned'),
            cents(data.amount),
            data.status,
            recurrence,
          ],
        ...
        const record = await db.get('SELECT * FROM maintenance WHERE id=$1 AND vehicle_id=$2', [
          integer(data.id, 1, Number.MAX_SAFE_INTEGER),
          vehicleId,
        ]);
        if (!record || record.status !== 'planned')
          fail('Manutenção não encontrada ou já realizada.', 409);
        const completedDate = date(data.date);
        const amount = cents(data.amount);
        await db.transaction(async (client) => {
          await client.query(
            "UPDATE maintenance SET status='done', date=$1, amount=$2 WHERE id=$3",
            [completedDate, amount, record.id],
          );
          if (record.recurrence) {
            const next = new Date(completedDate + 'T12:00:00Z');
            const day = next.getUTCDate();
            next.setUTCDate(1);
            next.setUTCMonth(next.getUTCMonth() + record.recurrence);
            const last = new Date(
              Date.UTC(next.getUTCFullYear(), next.getUTCMonth() + 1, 0),
            ).getUTCDate();
            next.setUTCDate(Math.min(day, last));
            await client.query(
              "INSERT INTO maintenance(vehicle_id,description,date,amount,status,recurrence) VALUES ($1,$2,$3,$4,'planned',$5)",
              [
                vehicleId,
                record.description,
      ...
      json(res, error.status || 400, {
        error: error.status
          ? error.message
          : 'Não foi possível salvar. Confira os campos e o tamanho das fotos.',
      });
    }
    return true;
  };
