...
const field = (label, name, value = '', attrs = '') =>
  `<label>${label}<input name="${name}" value="${esc(value)}" ${attrs}></label>`;
const mapsButton = (route) =>
  `<a class="secondary maps-button" href="${esc(route.maps_url)}" target="_blank" rel="noopener noreferrer">Abrir no Google Maps ↗</a>`;
const schedule = (route) =>
  route.weekdays.length ? route.weekdays.map((day) => days[day]).join(' · ') : 'Dias a definir';
const times = (route) =>
  `${route.departure || 'Saída a definir'}${route.arrival ? ` → ${route.arrival}${route.arrival_next_day ? ' (+1 dia)' : ''}` : ''}`;
function itinerary(route) {
  const points = [
    { name: route.origin, time: route.departure, notes: 'Origem' },
    ...route.stops,
    {
      name: route.destination,
      time: route.arrival,
      notes: `Destino${route.arrival_next_day ? ' · chegada no dia seguinte' : ''}`,
    },
  ];
...
export async function showRoutes(user, { api, shell }) {
  ...
  const message = (node, text, good = false) => {
    node.textContent = text;
    node.className = `form-message ${good ? 'success' : 'error'}`;
  };
  ...
  function detail(route, success) {
    ...
    if (data.canManage) {
      ...
      remove.textContent = 'Excluir rota';
      remove.onclick = () => deleteRoute(route, remove);
      document.querySelector('#edit-route').after(remove);
    }
  }
  async function deleteRoute(route, button) {
    if (
      !confirm(
        `Excluir a rota “${route.name}”?\nEla será removida da dashboard e da landing page. Esta ação não pode ser desfeita.`,
      )
    )
      return;
    ...
    let deleted = false;
    try {
      await api('routes/delete', { id: route.id, confirm_name: route.name });
      deleted = true;
      data.routes = data.routes.filter((item) => item.id !== route.id);
      activeId = null;
      draw('Rota excluída com sucesso.');
    } catch (error) {
      if (deleted) {
        location.href = '/rotas';
        return;
  ...
  function editor(route = null) {
    ...
    async function updateDistance() {
      collectStops();
      const signature = distanceSignature(),
        requestId = ++distanceRequest;
      const button = form.querySelector('#calculate-distance'),
        output = form.querySelector('#distance-message');
      button.disabled = true;
      output.textContent = 'Calculando distância…';
      try {
        const result = await api('routes/distance', { maps_url: form.elements.maps_url.value });
        if (!form.isConnected || requestId !== distanceRequest || signature !== distanceSignature())
          return;
        distanceField.value = String(result.distance).replace('.', ',');
        output.textContent = `${result.distance.toLocaleString('pt-BR')} km · OSRM / OpenStreetMap · pontos do link original. Pode diferir do Google Maps.`;
      } catch (error) {
        if (form.isConnected && requestId === distanceRequest) output.textContent = error.message;
      } finally {
        if (form.isConnected && requestId === distanceRequest) button.disabled = false;
      }
    }
    form.querySelector('#calculate-distance').onclick = updateDistance;
    form.addEventListener('input', (event) => {
      if (event.target.name === 'maps_url') {
        clearTimeout(distanceTimer);
        distanceRequest++;
        distanceField.value = '';
        form.querySelector('#calculate-distance').disabled = false;
        form.querySelector('#distance-message').textContent =
          'Percurso alterado. Atualizando a distância…';
        distanceTimer = setTimeout(() => {
          if (form.isConnected && form.elements.maps_url.value.trim()) updateDistance();
        }, 1200);
      }
    });
    function collectStops() {
      stops = [...document.querySelectorAll('.stop-editor')].map((node) => ({
    ...
    function renderStops() {
      document.querySelector('#route-stops').innerHTML =
        stops
          .map(
            (stop, i) =>
              `<div class="stop-editor"><div class="stop-editor-heading"><strong>Parada ${i + 1}</strong><div class="stop-controls"><button type="button" class="secondary" data-move="${i}" data-direction="-1" aria-label="Mover parada ${i + 1} para cima" ${i === 0 ? 'disabled' : ''}>↑</button><button type="button" class="secondary" data-move="${i}" data-direction="1" aria-label="Mover parada ${i + 1} para baixo" ${i === stops.length - 1 ? 'disabled' : ''}>↓</button><button type="button" class="text-button" data-remove-stop="${i}">Remover</button></div></div><div class="form-grid"><label>Local ou endereço<input data-field="name" value="${esc(stop.name)}" maxlength="300" required></label><label>Horário previsto<input data-field="time" value="${esc(stop.time)}" type="time"></label></div><label>Orientações da parada<input data-field="notes" value="${esc(stop.notes)}" maxlength="300" placeholder="Ex.: embarque em frente à portaria"></label></div>`,
          )
          .join('') ||
        '<p class="empty-state">Sem paradas intermediárias. O trajeto vai da origem ao destino.</p>';
      ...
      document.querySelectorAll('[data-move]').forEach(
        (button) =>
          (button.onclick = () => {
            ...
            renderStops();
            document.querySelectorAll('.stop-editor')[to].querySelector('input').focus();
          }),
      );
      document.querySelectorAll('[data-remove-stop]').forEach(
        (button) =>
          (button.onclick = () => {
            collectStops();
            stops.splice(Number(button.dataset.removeStop), 1);
            renderStops();
          }),
      );
    }
    renderStops();
    document.querySelector('#add-stop').onclick = () => {
      collectStops();
      if (stops.length >= 30) return;
      stops.push({ name: '', time: '', notes: '' });
      renderStops();
      document
        .querySelectorAll('.stop-editor input[data-field=name]')
        .item(stops.length - 1)
        .focus();
    };
    document.querySelector('#read-maps').onclick = async (event) => {
      ...
      try {
        ...
        if (parsed.requiresAddresses) {
          message(
            output,
            'O Maps trouxe coordenadas para alguns pontos. Preencha os endereços de origem, destino e paradas antes de salvar. Coordenadas não serão usadas como endereço.',
          );
          return;
        }
        if (!form.isConnected || form.elements.maps_url.value !== sourceLink) return;
        if (!parsed.origin || !parsed.destination) {
          message(
            output,
            'Link reconhecido. Origem, destino e paradas não estão legíveis neste link; preencha os campos abaixo.',
            true,
          );
          return;
        }
        collectStops();
        if (
          form.elements.origin.value.trim() ||
          form.elements.destination.value.trim() ||
          stops.length
        ) {
          message(
            output,
            'Para preservar seu roteiro, a leitura só preenche campos vazios. Limpe origem, destino e paradas se quiser substituí-los pelo link.',
          );
          return;
        }
        if (
          parsed.stops.length > 30 ||
          [parsed.origin, parsed.destination, ...parsed.stops].some((point) => point.length > 300)
        )
          throw new Error(
            'O link possui pontos demais ou endereços muito longos. Preencha o roteiro manualmente.',
          );
        form.elements.origin.value = parsed.origin;
        form.elements.destination.value = parsed.destination;
        stops = parsed.stops.map((name) => ({ name, time: '', notes: '' }));
        renderStops();
        updateDistance();
        message(
          output,
          'Pontos extraídos do endereço do link. Revise os locais e complete os horários antes de salvar.',
          true,
        );
      } catch (error) {
    ...
    form.onsubmit = async (event) => {
      ...
      const payload = {
        ...
        id: route?.id || null,
        stops,
        weekdays: [...form.querySelectorAll('[name=weekday]:checked')].map((input) =>
          Number(input.value),
        ),
        arrival_next_day: form.elements.arrival_next_day.checked,
      ...
      try {
        ...
        const result = await api('routes/save', payload);
        saved = true;
        activeId = result.id;
        data = await api('routes');
        draw('Rota salva com sucesso.');
      } catch (error) {
        if (saved) {
          location.assign(`/rotas?rota=${activeId}`);
          return;
        }
        message(document.querySelector('#route-save-message'), error.message);
        button.disabled = false;
      }
    };
  }
