export function isCoordinatePoint(value) {
  ...
  return (
    /^@?\s*[+-]?\d{1,3}(?:\.\d+)?\s*[,;]\s*[+-]?\d{1,3}(?:\.\d+)?(?:\s*,\s*\d+(?:\.\d+)?z)?\s*$/.test(
      text,
    ) ||
...
export function normalizeMapsEmbed(value) {
  ...
  if (typeof value !== 'string' || value.length > 20000)
    throw new Error('Código de mapa inválido.');
  const source = value.trim().startsWith('<')
    ? value.match(/\bsrc\s*=\s*["']([^"']+)["']/i)?.[1]
    : value.trim();
  let url;
  try {
    url = new URL(source?.replaceAll('&amp;', '&'));
  } catch {
    throw new Error('Cole o código de Incorporar um mapa fornecido pelo Google Maps.');
  }
  if (
    url.protocol !== 'https:' ||
    ...
    throw new Error('Use o código oficial do Google Maps: Compartilhar → Incorporar um mapa.');
  return url.href;
}
export function readMapsLink(value) {
  ...
  try {
    ...
    throw new Error('Cole o link completo do Google Maps, começando com https://.');
  }
  const domains = [
    'www.google.com',
    'google.com',
  ...
  if (url.protocol !== 'https:' || url.username || url.password || url.port || (!short && !normal))
    throw new Error('Use um link HTTPS do Google Maps (google.com/maps ou maps.app.goo.gl).');
  let origin = '',
    destination = '',
    stops = [];
  if (!short && url.searchParams.has('origin') && url.searchParams.has('destination')) {
    ...
    for (const segment of segments) {
      if (
        ...
        break;
      try {
        points.push(decodeURIComponent(segment.replaceAll('+', ' ')));
      } catch {
        break;
      }
    }
  ...
  return {
    url: url.href,
    short,
    origin: isCoordinatePoint(origin) ? '' : origin,
    destination: isCoordinatePoint(destination) ? '' : destination,
    stops: stops.map((point) => (isCoordinatePoint(point) ? '' : point)),
    requiresAddresses,
  };
}
