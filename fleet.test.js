...
import { fuelSummary, parseOdometer, maintenanceForecast } from './fleet.js';
...
test('quilometragem acima de 50 mil com formato brasileiro', () => {
  ...
  for (const value of ['', '-1', '150.00.0', '10000001', 'abc']) assert.ok(Number.isNaN(parseOdometer(value)));
});

test('média entre tanques completos, parciais e estimativas', () => {
  const rows = [{ full: 1, litres: 80, odometer: 1000, date: '2024-01-01' }, { full: 0, litres: 20, odometer: 1200 }, { full: 1, litres: 30, odometer: 1500, date: '2024-01-03' }];
...
test('frota: veículos isolados, custos, ARLA, agendamento e permissões', async () => {
  ...
  const get = async () => (await fetch(origin + '/api/fleet', { headers: { Cookie: cookie } })).json();
  const save = async (path, data, status = 201) => { const response = await post('fleet/' + path, data); assert.equal(response.status, status, JSON.stringify(await response.json())); };
  try {
    assert.equal((await fetch(origin + '/api/fleet')).status, 401);
    const signup = await post('setup', { name: 'Dono', email: 'dono@example.com', password: 'SenhaSeguraFrota123', token: process.env.SETUP_TOKEN });
    cookie = signup.headers.get('set-cookie').split(';')[0];
    const vehicle = { plate: 'ABC-1234', model: 'Ônibus executivo', year: 2024, seats: 46, odometer: 1000, fuel_capacity: 100, arla_capacity: 20, photos: [] };
    ...
    let fleet = await get(); const first = fleet.vehicles.find(v => v.plate === 'ABC1234'); const second = fleet.vehicles.find(v => v.plate === 'DEF1G23');
    ...
    const refill = { vehicle_id: first.id, kind: 'fuel', date: '2024-01-01', odometer: 1000, litres: 80, price: '6,199', full: true };
    ...
    await save('refill', { ...refill, date: '2024-01-02', odometer: 1200, litres: 20, price: '6,00', full: false });
    await save('refill', { ...refill, date: '2024-01-03', odometer: 1500, litres: 30, price: '6,00' });
    await save('refill', { ...refill, kind: 'arla', date: '2024-01-03', odometer: 1500, litres: 5, price: '4,00' });
    ...
    await save('entry', { vehicle_id: first.id, kind: 'revenue', date: '2024-01-03', description: 'Viagem', amount: '2000,00' });
    await save('entry', { vehicle_id: first.id, kind: 'fixed', date: '2024-01-03', description: 'Seguro', amount: '100,00' });
    await save('entry', { vehicle_id: first.id, kind: 'cost', date: '2024-01-03', description: 'Inválido', amount: '-10' }, 400);
    await save('maintenance', { vehicle_id: first.id, description: 'Troca de óleo', date: '2024-01-31', amount: '48,32', status: 'planned', recurrence: 1 });
    fleet = await get(); let v = fleet.vehicles.find(v => v.id === first.id);
    assert.equal(v.fuel.average, 10); assert.equal(v.fuel.estimated, 80);
    assert.equal(v.arla.average, 100); assert.equal(v.arla.estimated, 18);
    assert.equal(v.costs, 95592); assert.equal(v.fixed, 10000); assert.equal(v.revenue, 200000);
    assert.equal(fleet.vehicles.find(v => v.id === second.id).costs, 0);
    await save('maintenance/complete', { vehicle_id: second.id, id: v.maintenance[0].id, date: '2024-01-31', amount: '48,32' }, 409);
    await save('maintenance/complete', { vehicle_id: first.id, id: v.maintenance[0].id, date: '2024-01-31', amount: '48,32' });
    await save('maintenance/complete', { vehicle_id: first.id, id: v.maintenance[0].id, date: '2024-01-31', amount: '48,32' }, 409);
    fleet = await get(); v = fleet.vehicles.find(v => v.id === first.id);
    assert.equal(v.costs, 100424); assert.equal(v.profit, 99576);
    assert.deepEqual(v.maintenance_months['2024-01'], { planned: 0, actual: 4832, count: 0, total: 4832 });
    assert.deepEqual(v.maintenance_months['2024-02'], { planned: 4832, actual: 0, count: 1, total: 4832 });
    ...
    await save('entry', { vehicle_id: first.id, kind: 'revenue', description: 'Indevido', date: '2024-01-01', amount: 10 }, 403);
    ...
    for (const name of ['refills', 'maintenance', 'vehicle_entries']) assert.equal(db.prepare(`SELECT COUNT(*) AS total FROM ${name} WHERE vehicle_id=?`).get(first.id).total, 0);
    ...
    db.close();
    for (const path of ['/frota', '/fleet.js']) assert.equal((await fetch(origin + path)).status, 200);
