...
const input = (label, name, type = 'text', value = '', attrs = '') => `<label>${label}<input name="${name}" type="${type}" value="${esc(value)}" ${attrs} required></label>`;
const amountInput = (label, name, value = '') => input(label, name, 'text', value, 'inputmode="decimal" placeholder="0,00"');
const dateInput = (value = today(), future = false) => input('Data', 'date', 'date', value, future ? '' : `max="${today()}"`);
const submit = label => `<p class="form-message" role="status"></p><button class="primary" type="submit">${label}</button>`;
...
function table(headers, rows, empty = 'Nenhum registro.') {
  return rows.length ? `<div class="table-scroll"><table><thead><tr>${headers.map(h => `<th scope="col">${h}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table></div>` : `<p class="empty-state">${empty}</p>`;
}
async function photoData(file) {
  const response=await fetch('/api/media',{method:'POST',headers:{'Content-Type':file.type},body:file});const result=await response.json();if(!response.ok)throw new Error(result.error);return result.url;
}
export async function showFleet(user, { api, shell }) {
  ...
  const notice = (node, message, success = false) => { node.textContent = message; node.className = `form-message ${success ? 'success' : 'error'}`; };
  async function refresh(message = '') { state = await api('fleet'); draw(message); }
  function base(content) { shell(user, 'Frota', content); }
  ...
  function bindForm(form, endpoint, prepare = value => value, after = refresh) {
    form.onsubmit = async event => {
      ...
      try { const values = prepare(Object.fromEntries(new FormData(form))); await api(`fleet/${endpoint}`, { vehicle_id: activeId, ...values }); await after('Registro salvo com sucesso.'); }
      catch (error) { notice(message, error.message); button.disabled = false; }
    };
  }
  function vehicleForm(v = null, panel = null) {
    if (!panel) { base('<div class="page-heading"><div><h1>Adicionar veículo</h1><p>Cadastre os dados e as capacidades dos tanques.</p></div><a class="secondary" href="/frota">Voltar à frota</a></div><div id="vehicle-panel"></div>'); panel = document.querySelector('#vehicle-panel'); }
    let photos = [...(v?.photos || [])];
    if (!canManage) { panel.innerHTML = `<section class="card"><p>${v.seats} lugares · ${v.year} · ${esc(v.plate)}</p><p>Tanque: ${number(v.fuel_capacity)} L · ARLA: ${number(v.arla_capacity)} L</p><div class="vehicle-gallery">${photos.map(src => `<img src="${esc(src)}" alt="Foto de ${esc(v.model)}">`).join('')}</div></section>`; return; }
    ...
    const form = document.querySelector('#vehicle-form'); let processing = false;
    const gallery = () => {
      document.querySelector('#photo-gallery').innerHTML = photos.map((src, i) => `<div><img src="${esc(src)}" alt="Foto ${i + 1} do veículo"><button type="button" class="secondary" data-remove="${i}">Remover foto ${i + 1}</button></div>`).join('');
      document.querySelectorAll('[data-remove]').forEach(button => button.onclick = () => { if (processing) return; photos.splice(Number(button.dataset.remove), 1); gallery(); });
    };
    gallery();
    document.querySelector('#vehicle-photos').onchange = async event => {
      const files = [...event.target.files]; const message = form.querySelector('.form-message');
      if (photos.length + files.length > 5) { notice(message, 'Cada veículo pode ter no máximo cinco fotos.'); event.target.value = ''; return; }
      processing = true; form.querySelector('[type=submit]').disabled = true; event.target.disabled = true;
      try { const prepared = []; for (const file of files) prepared.push(await photoData(file)); photos.push(...prepared); gallery(); notice(message, 'Fotos prontas. Salve o veículo para confirmar.', true); }
      catch (error) { notice(message, error.message || 'Não foi possível ler a imagem.'); }
      finally { processing = false; form.querySelector('[type=submit]').disabled = false; event.target.disabled = false; event.target.value = ''; }
    };
    bindForm(form, 'vehicle', values => { if (processing) throw new Error('Aguarde o processamento das fotos.'); return { ...values, id: v?.id, photos }; }, async message => { if (!v) activeId = null; await refresh(message); });
    if (v) {
      ...
      trigger.onclick = () => { deleteForm.hidden = false; trigger.hidden = true; deleteForm.elements.confirm_plate.focus(); };
      document.querySelector('#cancel-delete').onclick = () => { deleteForm.hidden = true; trigger.hidden = false; deleteForm.reset(); trigger.focus(); };
      bindForm(deleteForm, 'delete', values => ({ confirm_plate: values.confirm_plate.trim().toUpperCase().replace(/[-\s]/g, '') }), async () => { activeId = null; await refresh('Veículo e registros excluídos com sucesso.'); });
    }
  }
  function refillPanel(v, panel) {
    panel.innerHTML = `<div class="fleet-work-grid">${canManage ? `<section class="card"><h2>Registrar abastecimento</h2><form id="refill-form"><label>Produto<select name="kind"><option value="fuel">Combustível</option>${v.arla_capacity > 0 ? '<option value="arla">ARLA</option>' : ''}</select></label>${dateInput()}${input('Quilometragem no abastecimento (km)', 'odometer', 'text', v.odometer, 'inputmode="decimal" maxlength="16" placeholder="Ex.: 150.000"')}${amountInput('Preço por litro (R$)', 'price')}${input('Litros abastecidos', 'litres', 'number', '', 'min="0.001" max="5000" step="0.001"')}<label class="checkbox-label"><input type="checkbox" name="full"> Tanque ficou completo</label><p class="hint" id="refill-total">Total: R$ 0,00</p>${submit('Salvar abastecimento')}</form></section>` : ''}<section class="card"><h2>Histórico de abastecimentos</h2><p class="hint">Registre em ordem de data e quilometragem. Combustível e ARLA têm médias independentes.</p>${table(['Data / produto', 'Km', 'Litros', 'R$/L', 'Total', 'Tanque'], v.refills.map(row => `<tr><td>${day(row.date)}<small class="cell-note">${row.kind === 'fuel' ? 'Combustível' : 'ARLA'}</small></td><td>${number(row.odometer)}</td><td>${number(row.litres)}</td><td>${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 3 }).format(row.price / 1000)}</td><td>${money(row.total)}</td><td>${row.full ? 'Completo' : 'Parcial'}</td></tr>`))}</section></div><div class="fuel-grid">${tank(v.fuel, 'Combustível', v)}${v.arla ? tank(v.arla, 'ARLA', v) : ''}</div>`;
    const form = document.querySelector('#refill-form'); if (!form) return;
    form.oninput = () => { const price = Number(form.elements.price.value.replace(',', '.')); const litres = Number(form.elements.litres.value); document.querySelector('#refill-total').textContent = `Total: ${Number.isFinite(price * litres) ? money(Math.round(Math.round(price * 1000) * litres / 10)) : '—'}`; };
  ...
  function maintenanceTable(v, onlyPlanned = false) {
    ...
    return table(['Serviço', 'Data', 'Valor', 'Situação'], records.map(row => `<tr><td>${esc(row.description)}${row.recurrence ? `<small class="cell-note">A cada ${row.recurrence} mês(es)</small>` : ''}</td><td>${day(row.date)}</td><td>${money(row.amount)}</td><td>${row.status === 'done' ? 'Realizada' : row.date < today() ? 'Agendada · vencida' : 'Agendada'}${canManage && row.status === 'planned' && !onlyPlanned ? `<button type="button" class="text-button complete-maintenance" data-id="${row.id}">Registrar realização</button>` : ''}</td></tr>`));
  }
  function maintenancePanel(v, panel) {
    ...
    document.querySelector('#maintenance-month').onchange = event => {
      if (!/^\d{4}-\d{2}$/.test(event.target.value)) { event.target.value = maintenanceMonth; return; }
      maintenanceMonth = event.target.value;
      document.querySelector('#monthly-maintenance').innerHTML = monthlyMaintenance(v, maintenanceMonth);
    };
    const form = document.querySelector('#maintenance-form');
    if (form) {
      form.elements.status.onchange = () => { const done = form.elements.status.value === 'done'; form.elements.recurrence.disabled = done; if (done) { form.elements.recurrence.value = '0'; form.elements.date.max = today(); } else form.elements.date.removeAttribute('max'); };
      bindForm(form, 'maintenance', values => ({ ...values, recurrence: values.recurrence || 0 }));
    }
    document.querySelectorAll('.complete-maintenance').forEach(button => button.onclick = () => {
      const row = v.maintenance.find(item => item.id === Number(button.dataset.id));
      document.querySelector('#completion-panel').innerHTML = `<form id="complete-form" class="completion-form"><h3>Concluir: ${esc(row.description)}</h3>${dateInput()}${amountInput('Valor efetivamente pago (R$)', 'amount', (row.amount / 100).toFixed(2).replace('.', ','))}${submit('Confirmar realização')}</form>`;
      bindForm(document.querySelector('#complete-form'), 'maintenance/complete', values => ({ ...values, id: row.id }));
      document.querySelector('#complete-form input').focus();
    });
  }
  function financePanel(v, panel) {
    panel.innerHTML = `<section class="card">${metrics(v)}<p class="hint">Custos totais = abastecimentos + ARLA + manutenções realizadas + custos fixos + outros custos.</p></section><div class="fleet-work-grid finance-work">${canManage ? `<section class="card"><h2>Novo lançamento</h2><form id="entry-form"><label>Tipo<select name="kind"><option value="revenue">Faturamento</option><option value="fixed">Custo fixo</option><option value="cost">Outro custo</option></select></label>${input('Descrição', 'description', 'text', '', 'maxlength="120" placeholder="Ex.: viagem, seguro, IPVA"')}${dateInput()}${amountInput('Valor (R$)', 'amount')}<p class="hint">Lance cada pagamento de custo fixo na data em que ocorreu. Não repita aqui abastecimentos ou manutenções já registrados.</p>${submit('Salvar lançamento')}</form></section>` : ''}<section class="card"><h2>Faturamento e outros custos</h2>${table(['Data', 'Descrição', 'Tipo', 'Valor'], v.entries.map(row => `<tr><td>${day(row.date)}</td><td>${esc(row.description)}</td><td>${types[row.kind]}</td><td>${money(row.amount)}</td></tr>`))}</section></div>`;
    const form = document.querySelector('#entry-form'); if (form) bindForm(form, 'entry');
  }
  draw();
}
